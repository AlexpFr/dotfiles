...
case "${TYPE}" in
  ...
  scp)
    ...
    echo "Methode $TYPE, run the folowings commands:"
    echo "ssh root@${PROXMOX_IP}"
    echo "bash /root/custom_proxmox_config.sh '${PROXMOX_USER}' '${USER_PWD_HASH}'"
    ;;
  curl)
    echo "Methode $TYPE, run the folowings commands:"
    echo "ssh root@${PROXMOX_IP}"
    echo "url=https://raw.githubusercontent.com/AlexpFr/dotfiles/main/sys_install/proxmox/custom_proxmox_config.sh"
    echo "curl -fsSL "\$url" | bash -s -- '${PROXMOX_USER}' '${USER_PWD_HASH}'"
    ;;
  *)
